...
linear_fit_plot_pvalue_format(AUC_ORR_compare$PRISM,AUC_ORR_compare$ORR,rownames(AUC_ORR_compare), title="PRISM vs ORR",ylab="Clinical Response (%)",xlab="In vitro Response (AUC)")
...
linear_fit_plot_pvalue_format((prism_avg[names(prism_avg)]),orr_avg[names(prism_avg)],names(prism_avg),
...
linear_fit_plot_pvalue_format((prism_avg[overlap_drugs]),Cmax_vector[overlap_drugs],overlap_drugs,
                              title="PRISM vs Cmax",ylab="Clinical Cmax Log10(uM)",xlab="In vitro Response (AUC)",ylim=c(-3,3))



###############
#ADD CMAX TO AUC_ORR_compare:
###############

AUC_ORR_compare$Cmax<-NA
for (drug in unique(AUC_ORR_compare$drug)){
  if (drug %in% names(Cmax_vector)){
    drug_idx<-which(AUC_ORR_compare$drug==drug)
    AUC_ORR_compare$Cmax[drug_idx]<-Cmax_vector[drug]
  }
}

#NEED TO RE-NORMALIZE LOG-SCALE range to 0-1 (so apples-to-apples w/ AUC)
AUC_ORR_compare$C_norm1<-(AUC_ORR_compare$Cmax - min(AUC_ORR_compare$Cmax, na.rm = TRUE)) / (max(AUC_ORR_compare$Cmax, na.rm = TRUE) - min(AUC_ORR_compare$Cmax, na.rm = TRUE))
...
Cmax_vector<-Cmax_vector[which(is.na(Cmax_vector)==F)]

names(prism_avg)[-which(names(prism_avg) %in% names(Cmax_vector))]
...
#PLOT ALL VALUES
linear_fit_plot_pvalue((AUC_ORR_compare$AUC_Cmax),AUC_ORR_compare$ORR,rownames(AUC_ORR_compare), title="PRISM vs ORR",ylab="Clinical Response (%)",xlab="In vitro Response (AUC/TRmin)")
